 ...
 */
public class TCPIMSService extends AbstractIMSService implements IMSInterface {

    private static volatile TCPIMSService instance;

    private TCPIMSService() {
    }

    public static TCPIMSService getInstance() {
        if (null == instance) {
            synchronized (TCPIMSService.class) {
                if (null == instance) {
                    instance = new TCPIMSService();
                }
            }
        }
    ...
    public void sendMsg(IMSMsg msg) {
        ...
        if (transferProtocol == TransferProtocol.Protobuf) {
            ...
            channel.writeAndFlush(MsgBuilder.convertToJsonMsg(msg));
        }
    }
                ...
                for (int j = 1; j <= configuration.getReconnectCount(); j++) {
                    ...
                    try {
                        ...
                        if (channel != null && channel.isActive() && channel.isOpen() && channel.isRegistered()) {
                            Log.d(IMSConfiguration.TAG, String.format("连接Server『%s %d』成功，准备进行认证...", host, port));
    ...
    private void toServer(String host, int port) {
        try {
            ...
            Log.e(IMSConfiguration.TAG, String.format("连接Server『%s %d』失败", host, port));
            channel = null;
        }
    }
}
